...
## 2. Overview
Bài này có một form đăng nhập, và challenge cung cấp source của trang đăng nhập này, khi khai thác sẽ phát hiện ra lỗ hổng SQLi và 1 hàm xử lý chuỗi đầu vào ngăn chặn `'` và `"`. Cách chặn này khá giống bài `no quotes` ([[../no quotes/README|README]]) nhưng ở bài này sẽ phức tạp hơn đôi chút vì có hàm so sánh giá trị input của người dùng với output của câu SQL query. Có thể bypass bằng cách dùng kỹ thuật `SQL Quine` kết hợp với cách bypass của bài trước ta sẽ lấy được flag của bài này.
## 3. Reconnaissance
Truy cập vào trang web và thấy form đăng nhập.
Nhập thử một username và password bất kỳ: `test`/`test.

![[images/Pasted image 20260113121405.png]]
...
![[images/Pasted image 20260113121959.png]]

Thử nhập một account khác thì web trả về:

![[images/Pasted image 20260113121944.png]]
